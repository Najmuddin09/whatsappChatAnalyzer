...
def fetch_stats(selected_user, df):
    ...
    # 3. No.of media
    num_of_media = msg = df[df['messages'] == '<Media omitted>\n'].shape[0]
    # 4. fetch no.of links
    links = []
...
def most_busy_users(df):
    ...
    df = (df['user'].value_counts() / df.shape[0] * 100).reset_index().rename(
...
def activity_heatmap(selected_user, df):
    if selected_user != "Overall":
        df = df[df['user'] == selected_user]
    user_heatmap = df.pivot_table(index='day_name', columns='range', values='messages', aggfunc='count').fillna(0)
    return user_heatmap
